# RESULT: bad. opacity_reset_interval=1000 also divides coarse_iterations(1000), so the fine stage
# starts from a just-flattened model. Lost ~2dB even at the original view. Keep interval >1000.
...
OptimizationParams = dict(
    ...
    percent_dense = 0.01,
    opacity_reset_interval = 1000,       # was 3000, which never fired at 2000 iters. Fires end-of-coarse and fine iter 1000; must stay >size_prune_grace_period(500) or size pruning never re-arms
...
RuntimeParams = dict(
    log_point_counts = True,             # confirm the resets fire and show what they cost
)
