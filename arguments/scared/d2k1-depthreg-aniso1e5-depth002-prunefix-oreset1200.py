# RESULT: mixed. Removes the wide-angle collapse -- elev30 seed spread 4.60 -> 0.72, elev45 +2.4dB
# -- but costs the clean elev10-20 wins. Use if consistency matters more than peak numbers.
...
OptimizationParams = dict(
    ...
    percent_dense = 0.01,
    opacity_reset_interval = 1200,       # coarse only reaches 1000 so it never resets; fine resets once at 1200 with 800 iters to recover. Must stay >size_prune_grace_period(500) or size pruning never re-arms
...
RuntimeParams = dict(
    log_point_counts = True,             # confirm the reset fires and show what it costs
)
