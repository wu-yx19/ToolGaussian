...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, stage="fine"):
    ...
    # 3D points projected to 2D, not image points
    ...
    # print("means3D:", means3D_final.min().item(), means3D_final.max().item())
    # print("scales:", scales_final.min().item(), scales_final.max().item())
    # print("opacity:", opacity_final.min().item(), opacity_final.max().item())

    # print("camera center:", viewpoint_camera.camera_center)
    # print("view matrix norm:", viewpoint_camera.world_view_transform.norm())
    ...
    return {"render": rendered_image,
            "depth": depth,
            "alpha": alpha, # accumulated per-pixel opacity (1 - transmittance); diagnostic only, no gradient
            "viewspace_points": screenspace_points,
            "visibility_filter" : radii > 0,
            "radii": radii,} # bounding radius
