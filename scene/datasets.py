...
from scene.views import View
from typing import NamedTuple
from utils.math_utils import percentile_torch
from utils.graphics_utils import getWorld2View2, focal2fov, BasicPointCloud, storePly, fetchPly
import glob
...
class SceneInfo(NamedTuple): # return package
    point_cloud: BasicPointCloud
    train_views: list
    test_views: list
    video_views: list
    nerf_normalization: dict
    # ply_path: str # point cloud path
    maxtime: int

def getNerfppNorm(cam_info): # return translation and scene scale
...
class EndoNeRF_Dataset(object):
    def __init__(
        ...
        self.video_idxs = [i for i in range(n_frames)] # all frames
    ...
    def load_meta(self):
        ...
        poses = poses_arr[:, :-2].reshape([-1, 3, 5])  # (N_cams, 3, 5) [R|t|hwf]
    ...
    def format_infos(self, split):
        ...
        for idx in tqdm(idxs):
            ...
            if self.mode == 'binocular':
                ...
                depth[depth!=0] = (1 / depth[depth!=0])*0.4 # inverse depth
            ...
            cameras.append(View(colmap_id=idx,R=R,T=T,FoVx=FovX,FoVy=FovY,image=image, depth=depth, mask=mask, gt_alpha_mask=None,
    ...
    def get_pts_cam(self, depth, mask, color, disable_mask=False):
        ...
        if not disable_mask:
            ...
            color_valid = color[mask, :] # rule out masked points
        else:
            mask = mask.reshape(-1).astype(bool)
            pts_valid = pts_cam
            color_valid = color
    ...
    def get_scene_info(self):
        ...
        train_cam_infos = self.format_infos(split="train")
        test_cam_infos = self.format_infos(split="test")
        video_cam_infos = self.format_infos(split="video")
        # get normalizations
        ...
        scene_info = SceneInfo(point_cloud=pcd,
                               train_views=train_cam_infos, # data
                               test_views=test_cam_infos,
                               video_views=video_cam_infos,
...
class SCARED_Dataset(object):
    ...
    def load_meta(self):
        ...
        """
        # prepare paths
        calibs_dir = os.path.join(self.root_dir, "data", "frame_data")
        rgbs_dir = os.path.join(self.root_dir, "data", "left_finalpass")
        disps_dir = os.path.join(self.root_dir, "data", "disparity")
        monodisps_dir = os.path.join(self.root_dir, "data", "left_monodam")
        reproj_dir = os.path.join(self.root_dir, "data", "reprojection_data")
        ...
        for i_frame in trange(n_frames, desc="Process frames"):
            ...
            with open(os.path.join(calibs_dir, f"{frame_id}.json"), "r") as f:
            ...
            # rgbs and depths
            rgb_dir = os.path.join(rgbs_dir, f"{frame_id}.png")
            rgb = iio.imread(rgb_dir)
            rgbs.append(rgb)

            if self.mode == 'binocular':
                disp_dir = os.path.join(disps_dir, f"{frame_id}.tiff")
                disp = iio.imread(disp_dir).astype(np.float32)
                h, w = disp.shape
                with open(os.path.join(reproj_dir, f"{frame_id}.json"), "r") as json_file:
                ...
                # disp_dir = os.path.join(monodisps_dir, f"{frame_id}_depth.png")
                ...
                disp_dir = os.path.join(monodisps_dir, f"{frame_id}.png")
    ...
    def get_scene_info(self): # datadir, mode, init_pts
        # load camera infos
        train_cam_infos = self.format_infos(split="train")
        test_cam_infos = self.format_infos(split="test")
        video_cam_infos = self.format_infos(split="video")
...
class Hamlyn_Dataset(object):
    ...
    def get_scene_info(self):
        ...
        test_cam_infos = self.format_infos(split="test")
        video_cam_infos = self.format_infos(split="video")
        # interp_cam_infos = self.format_infos(split='interp')
