...
def run_scale_opacity_check(
    ply_path,
    out_dir=None,
    bins=100,
    extent=10.0,
    scale_extent_ratio=0.1,
    opacity_threshold=0.005,
    opacity_reset_value=0.01,
):
    out_dir = out_dir if out_dir else infer_out_dir(ply_path)
    os.makedirs(out_dir, exist_ok=True)

    scales, opacity = load_gaussian_properties(ply_path)  # scales: (N, 3) real-world magnitudes, opacity: (N,) in [0,1]
    max_scale = scales.max(axis=1)
    min_scale = scales.min(axis=1)
    ratio = max_scale / min_scale

    size_threshold = scale_extent_ratio * extent
    pct_above_size_threshold = 100.0 * np.mean(max_scale > size_threshold)
    pct_below_opacity_threshold = 100.0 * np.mean(opacity < opacity_threshold)
    pct_below_opacity_reset_value = 100.0 * np.mean(opacity < opacity_reset_value)
    ...
    lines = [
        ...
        f"world-space size threshold = scale_extent_ratio({scale_extent_ratio}) * extent({extent}) = {size_threshold:.4g} "
        f"-> {pct_above_size_threshold:.2f}% of points would be pruned by this criterion alone",
        f"opacity_threshold={opacity_threshold} -> {pct_below_opacity_threshold:.2f}% of points already below it",
        f"opacity_reset_value={opacity_reset_value} -> {pct_below_opacity_reset_value:.2f}% of points already below it "
        f"(i.e. would be unaffected by a reset_opacity() ceiling at this value)",
    ]
    for line in lines:
        print(line)

    log_name = os.path.splitext(os.path.basename(ply_path))[0] + "_scale_opacity_check.log"
    ...
    axes[0].hist(log_ratio, bins=bins, color="steelblue")
    ...
    axes[1].hist(log_max_scale, bins=bins, color="darkorange")
    ...
    axes[2].hist(opacity, bins=bins, color="seagreen", range=(0, 1))
    axes[2].axvline(np.median(opacity), color="red", linestyle="--", linewidth=1,
                     label=f"median={np.median(opacity):.3g}")
    axes[2].axvline(opacity_threshold, color="black", linestyle=":", linewidth=1.5,
                     label=f"opacity_threshold={opacity_threshold} ({pct_below_opacity_threshold:.2f}% below)")
    axes[2].axvline(opacity_reset_value, color="purple", linestyle=":", linewidth=1.5,
                     label=f"opacity_reset_value={opacity_reset_value} ({pct_below_opacity_reset_value:.2f}% below)")
    axes[2].set_xlabel("opacity")
    axes[2].set_ylabel("Gaussian count")
    axes[2].set_title("Opacity distribution")
    axes[2].legend()

    fig.suptitle(os.path.basename(ply_path))
    fig.tight_layout()
    out_name = os.path.splitext(os.path.basename(ply_path))[0] + "_scale_opacity_check.png"
...
def main():
    ...
    run_scale_opacity_check(
        args.ply_path,
        out_dir=args.out_dir,
        bins=args.bins,
        extent=args.extent,
        scale_extent_ratio=args.scale_extent_ratio,
